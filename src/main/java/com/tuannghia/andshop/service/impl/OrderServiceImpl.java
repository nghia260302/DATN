...
import com.tuannghia.andshop.constant.OrderStatus;
import com.tuannghia.andshop.constant.PaymentMethod;
...
import com.tuannghia.andshop.entity.*;
import com.tuannghia.andshop.repository.ClotheRepository;
import com.tuannghia.andshop.repository.OrderDetailRepository;
import com.tuannghia.andshop.repository.OrderRepository;
import com.tuannghia.andshop.repository.ProductSizeRepository;
...
public class OrderServiceImpl implements OrderService {
    ...
    public void setDeliveringOrder(Order order) {
        ...
        for (OrderDetail orderDetail : orderDetailList) {
            orderDetail.getProductSize().setBuyCount(orderDetail.getProductSize().getBuyCount() + orderDetail.getQuantity());
            orderDetail.getProductSize().setQuantity(orderDetail.getProductSize().getQuantity() - orderDetail.getQuantity());
    ...
    public Order createUnpaidOrder(CartDTO cart, User user, OrderPerson orderPerson) {
        ...
        for (CartItemDTO cartItem : cartItems) {
            ...
            ProductSize productSize = productSizeRepository.findById(cartItem.getProductSizeId()).orElse(null);
            orderDetail.setProductSize(productSize);
            orderDetail.setQuantity(cartItem.getQuantity());
            assert productSize != null;
            orderDetail.setPrice(productSize.getClothe().getSalePrice());
            order.addOrderDetail(orderDetail);
            double currentTotalRevenue = productSize.getTotalRevenue();
            if (productSize.getTotalRevenue() == 0) {
                currentTotalRevenue = (double) 0;
            }
            productSize.setTotalRevenue(currentTotalRevenue + (double) orderDetail.getPrice());
        }
